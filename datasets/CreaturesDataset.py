...
RAW_DIR = os.path.join("../", 'data/pilot_coll1/')
...
class CreaDataset(data.Dataset):
    def __init__(self, vocab=None, split='Train', context_condition='all', 
        ...
        print('loading CSV')
        if (self.split == "Train"):
            csv_path = os.path.join(RAW_DIR, 'train/msgs.tsv')
            csv_path_concat = os.path.join(RAW_DIR, 'train/msgs_concat.tsv')
            csv_path_data = os.path.join(RAW_DIR, 'train/vision/dataset.tsv')

        if (self.split == "Validation"):
            csv_path = os.path.join(RAW_DIR, 'val/msgs.tsv')
            csv_path_concat = os.path.join(RAW_DIR, 'val/msgs_concat.tsv')
            csv_path_data = os.path.join(RAW_DIR, 'val/vision/dataset.tsv')

        if (self.split == "Test"):
            csv_path = os.path.join(RAW_DIR, 'test/msgs.tsv')
            csv_path_concat = os.path.join(RAW_DIR, 'test/msgs_concat.tsv')
            csv_path_data = os.path.join(RAW_DIR, 'test/vision/dataset.tsv')
        
        df = pd.read_csv(csv_path,sep='\t', header=0)
        df = df[df['sender'] == "speaker"]

        df_concat = pd.read_csv(csv_path_concat,sep='\t', header=0)
        df_data = pd.read_csv(csv_path_data,sep='\t', header=0)

        df_msg_concat = df_concat[['message']]
        concat_msg_np = np.asarray(df_msg_concat)        
        breakpoint()      

        df_msg_data = df_data[['message']]

        breakpoint()
        vision_np = np.asarray(df_msg_data) 
        list1 = np.array([])
        for i, vision_msg in enumerate(vision_np):
            text = ""
            for x, concat_msg in enumerate(concat_msg_np):
                if (vision_msg == concat_msg):
                    id = df_concat[df_concat['message'] == concat_msg[0]]['example_id']
                    if (len(id) is not 0):
                        print(id.item())
                        df1 = df[df['example_id'] == id.item()]
                        df1 = df1['message']
                        for i in df1:
                            text += i
                        np.append(list1,[text, df_data['distr1'], df_data['distr2'], df_data['target']])
